...
void EnvelopeComponent::mouseDoubleClick(const juce::MouseEvent& e)
{
    ...
    pointDragging = newPoint.get();
    points.insert(it, std::move(newPoint));
    
    repaint();
}

void EnvelopeComponent::setGridDivisions(int vertical, int horizontal)
{
...
void EnvelopeComponent::setWaveformColour(juce::Colour colour)
{
...
void EnvelopeComponent::setScrollDirection(bool leftToRight)
{
    waveformScrollLeftToRight = leftToRight;
    waveformNeedsRedraw.store(true);
}

void EnvelopeComponent::toggleScrollDirection()
{
    waveformScrollLeftToRight = !waveformScrollLeftToRight;
    waveformNeedsRedraw.store(true);
}

bool EnvelopeComponent::isScrollingLeftToRight() const
{
    return waveformScrollLeftToRight;
}
...
float EnvelopeComponent::distanceToLineSegment(const juce::Point<float>& p, const juce::Point<float>& v, const juce::Point<float>& w) const
{
